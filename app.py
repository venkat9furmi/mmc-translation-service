...
def detect_table_type(table):
    """
    Detects table type from its HEADER ROW content, not its index.

    Some scripts have a dedicated running-order table first
    (header: No/Item/Dur/R/T) — others start straight into a content table
    (header: TIME/PICTURE:SOUND/COMMENTARY) with no running order at all.
    Assuming "table 0 = running order" silently drops Table 0's real
    commentary on documents that don't have one.

    Used by BOTH build_cell_map (read path) and the /translate apply loop
    (write path) — kept as a single shared function so the two paths can
    never drift out of sync with each other again.
    """
    if not table.rows:
        return False, False
    header = [c.text.strip().upper() for c in table.rows[0].cells]
    is_running_order = any("ITEM" in h or "DUR" in h for h in header)
    is_content_table = any("COMMENTARY" in h for h in header)
    return is_running_order, is_content_table
...
def build_cell_map(doc):
    ...
    for t_idx, table in enumerate(doc.tables):
        is_running_order, is_content_table = detect_table_type(table)

        for r_idx, row in enumerate(table.rows):
            n_cols = len(row.cells)

            # Running order table: translate FEATURE descriptions in col 1
            if is_running_order and n_cols >= 2:
                text = cell_text(row.cells[1])
                if not should_skip(text) and "FEATURE" in text.upper():
                    cell_map[f"T{t_idx}R{r_idx}C1"] = {"text": text, "label": ""}

            # Content table: translate COMMENTARY col (usually col 3)
            # label = column 1 of the SAME row (PICTURE:SOUND / SOT tag)
            elif is_content_table and n_cols >= 4:
...
def parse_table_map_response(output, keys):
    ...
    NOTE: the model includes its own "DE:"/"FR:" prefix in the output
    (as instructed). We strip it here so it's stored clean — otherwise
    add_translation_to_cell() prepends the prefix AGAIN, producing the
    "DE: DE: ..." duplicate bug.
    """
    translations = {}
    key_set = set(keys)
    ...
    for line in output.splitlines():
        ...
        if possible_key in key_set:
            rest = rest.strip()
            # Strip a leading DE:/FR: prefix the model included, if present
            rest = re.sub(r"^(DE|FR):\s*", "", rest)
            translations[possible_key] = rest
...
def translate_full_script(cell_map, language, api_key):
    ...
    for i, start in enumerate(range(0, len(all_keys), BATCH_SIZE)):
        ...
        parsed     = parse_table_map_response(output, batch_keys)
        translations.update(parsed)

        if len(parsed) < len(batch_keys):
            print(f"[DEBUG] Batch {i}: expected {len(batch_keys)} keys, "
                  f"parsed {len(parsed)}. Raw output sample: {output[:300]!r}",
                  flush=True)

    # ── Pass 2: backfill any keys the model silently dropped ─────────────
    # IMPORTANT: chunk the backfill too — dumping ALL missing keys into one
    # call is what caused the original 413 (a large miss-count produced a
    # backfill prompt big enough to blow past the per-minute token limit).
    for round_num in range(MAX_BACKFILL_ROUNDS):
        missing = [k for k in all_keys if k not in translations]
        if not missing:
            break
        print(f"[DEBUG] Backfill round {round_num}: {len(missing)} keys missing",
              flush=True)
        for start in range(0, len(missing), BATCH_SIZE):
            time.sleep(2)
            chunk_keys = missing[start : start + BATCH_SIZE]
            batch      = {k: cell_map[k] for k in chunk_keys}
            prompt     = build_single_prompt(batch, language)
            output     = call_groq(prompt, api_key)
            translations.update(parse_table_map_response(output, chunk_keys))
...
def health():
    return jsonify({
        "status": "ok",
        "service": "MMC Sport Translation Service v2",
        "code_version": "2026-06-23-unified-detection-v4",
    })
